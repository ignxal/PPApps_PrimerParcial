...
export class AuthFirebaseService {
  ...
  async login(user: User) {
    let userDos = user;
    return signInWithEmailAndPassword(this.auth, user.email, user.pass).then(
      async (userCredential) => {
        const userAuth = userCredential.user;
        console.log('Usuario logueado -> ', userAuth);

        await this.collection
          .getOne(this.collectionName, userAuth.uid)
          .then((user) => {
  ...
  async addUser(user: Person, password: string): Promise<boolean> {
    const userAdd = await createUserWithEmailAndPassword(
      ...
      .then(async (userCredential) => {
        ...
        this.collection
          .addOne(this.collectionName, user)
          .then((e) => {
