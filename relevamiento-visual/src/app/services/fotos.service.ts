...
export class FotosService {
  ...
  uploadFoto(foto: Foto) {
    return this.collections.addOne(this.collectionName, foto).then((data) => {
      return data;
    });
  }
  ...
  getFotos(tipo: TipoCosa) {
    ...
    return this.collections.getAllWhereSnapshot<Foto>(
      this.collectionName,
      and(...query),
      'fecha',
      by
  ...
  getFotosByUser(email: string) {
    let query: QueryFilterConstraint[] = [];
    let by: OrderByDirection = 'desc';

    query.push(where('email', '==', email));
